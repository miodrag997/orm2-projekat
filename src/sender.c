...
uint8_t src_wifi_ip_address[4] = { 192, 168, 64, 146 };
...
uint8_t dst_wifi_ip_address[4] = { 192, 168, 64, 209 };
/* PAVLE
uint8_t dst_eth_mac_address[6] = { 0x2c, 0x59, 0xe5, 0x04, 0xcd, 0x50 };
uint8_t dst_eth_ip_address[4] = { 192, 168, 0, 12 };
uint8_t dst_wifi_mac_address[6] = { 0x8c, 0xa9, 0x82, 0x15, 0x28, 0x36 };
uint8_t dst_wifi_ip_address[4] = { 172, 20, 10, 2 };
*/
...
void loop_handler(unsigned char *param, const struct pcap_pkthdr *packet_header, const unsigned char *packet_data) {
	...
	// TODO: ntohs?
	//if (p->data[0] == 'A' && p->data[1] == 'C' && p->data[2] == 'K' && pack == ntohs(p->cus_h.seq_no)) {
	...
	while (next_packet < packet_no) {
		...
		while (!received[pack]) {
			if (wifi_alive) {
				...
				pcap_loop(wifi_device_handle, 1, loop_handler, (unsigned char*) &pack);
				retries++;
				if (received[pack]) {
					retries = 0;
				}
				...
				pcap_loop(eth_device_handle, 1, loop_handler, (unsigned char *) &pack);
			} else {
		...
		free(p);
	}for (int pack = 0; pack < packet_no; pack++) {
		int retries = 0;
		while (!received[pack]) {
			ethernet_header eh = create_eth_header(dst_wifi_mac_address, src_wifi_mac_address);
			ip_header ih = create_ip_header(MAX_DATA_SIZE, src_wifi_ip_address, dst_wifi_ip_address);
...
void *eth_thread_function(void *param) {
	...
	while (next_packet < packet_no) {
		...
		int pack = next_packet++;
		pthread_mutex_unlock(&mutex);
		debug("next_packet=%d, packet_no=%d\n", next_packet, packet_no);
